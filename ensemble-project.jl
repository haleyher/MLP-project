...
# ╔═╡ ef28983c-2b0c-11ee-1813-7ddbb6df838c
using Images, CSV, Plots, DataFrames, DataSets, Flux, JuliaHubData, GLM, MLJ,CombineML

# ╔═╡ da10c4bf-be2e-4236-8a12-cb9f4806d5d6
# import CombineML

# ╔═╡ d9d7e161-36ab-4787-8ecc-6a124b0699f1
galaxies = open(Vector{UInt8}, dataset("honih0518/galaxies2")) do buf
	CSV.read(buf, DataFrame)
end

# ╔═╡ 7eeb70be-3fda-4ecd-aaca-7f402b6313c2
data_train, data_test = MLJ.partition(galaxies, 0.75, shuffle = true)

# ╔═╡ af58d99c-65f5-4e1f-b1f8-e9fe58fb4756
...
# ╔═╡ 76b79a7f-17c4-4585-a02d-dbd6264a2282


# ╔═╡ f25260a6-be14-4c6a-be42-c58967e89466
size(galaxies)

# ╔═╡ 54d94e4c-ca59-4db4-bcf3-024c9508ba52
nfeatures = size(galaxies)[2] - 1 

# ╔═╡ 41628c90-6501-4b25-b404-51a1bf78f773
begin
	## training set
...
# ╔═╡ 0fdae020-8920-4b9d-ab51-f6142afdb6cc
begin
	## test set
...
# ╔═╡ 889867f8-6314-48d5-b181-b4f625e3a75f
begin
# building the model
nlabels = 2
insize, outsize = nfeatures, nlabels - 1

#model = Chain(Dense(insize, 15, σ),
model = Chain(Dense(insize, 15, relu; init = Flux.glorot_normal),
		# first connection: nfeatures inputs, 4 outputs
		Dense(15,15, relu; init = Flux.glorot_normal), # complete
		# second connection: 4 inupts, 6 outputs
		Dense(15,1, σ; init = Flux.glorot_normal)) # complete
		#third connection: 6 inputs, 1 output
end

# ╔═╡ b671c5d1-2cf2-4227-ab3b-fc5bf44730d0
begin
# building the model
nulabels = 2
ins, outs = nfeatures, nlabels - 1

#model = Chain(Dense(insize, 15, σ),
model2 = Chain(Dense(ins, 25, σ),
		# first connection: nfeatures inputs, 4 outputs
		Dense(25,15,σ), # complete
		# second connection: 4 inupts, 6 outputs
		Dense(15,1, σ)) # complete
		#third connection: 6 inputs, 1 output
end

# ╔═╡ c3147089-c2de-4a90-b2c5-c4a3466fa98a
begin
# building the model
nlabels3 = 2
ins3, outs3 = nfeatures, nlabels - 1

#model = Chain(Dense(insize, 15, σ),
model3 = Chain(Dense(ins3, 35, σ),
		# first connection: nfeatures inputs, 4 outputs
		Dense(35,15,σ), # complete
		# second connection: 4 inupts, 6 outputs
		Dense(15,1, σ)) # complete
		#third connection: 6 inputs, 1 output
end

# ╔═╡ fd110384-495d-4fd5-8ffc-1acf410fe8a5
# begin 
# 	# defining loss	
...
# ╔═╡ 1f4fab68-59bb-48fd-973b-af3748c62dd3
begin 
	# defining loss	
	loss(m, x, y) = Flux.logitbinarycrossentropy(first.(m.(x)), y)
    # broadcast operation: m is evaluated in each component of the vector 
	...
	# optimizer = complete (use gradient descent with a learning rate of 0.2)
	optimizer = Flux.setup(Adam(3e-4), model)
	 n_epochs = 700
end	

# ╔═╡ 349ae7b3-200f-4abe-9d7f-2dbc4833db76
begin 
	# defining loss	
	loss2(m, x, y) = Flux.logitbinarycrossentropy(first.(m.(x)), y)
    # broadcast operation: m is evaluated in each component of the vector 

	# optimizer = complete (use gradient descent with a learning rate of 0.2)
	optimizer2 = Descent(0.2)
	 n_epochs2 = 700
end	

# ╔═╡ 554c9963-ffe3-4bb9-a0a4-6f92148c4874
begin 
	# defining loss	
	loss3(m, x, y) = Flux.logitbinarycrossentropy(first.(m.(x)), y)
    # broadcast operation: m is evaluated in each component of the vector 

	# optimizer = complete (use gradient descent with a learning rate of 0.2)
	optimizer3 = Descent(0.2)
	 n_epochs3 = 900
end	

# ╔═╡ 381f9174-c6f2-476c-a032-56939f703eff
begin
...
# ╔═╡ f01b1894-3b7b-4d89-baaa-e088a3b4129d
begin
	# training (pay attention to the syntax)
	
	losses2 = [] # history of losses
	
	for _ in 1:n_epochs
	    
	    # complete training
	    Flux.train!(loss2, model2, data_train_mlp, optimizer2)
	    # Accumulate losses
	    l = loss2(model2, x_train, y_train)
	    push!(losses2, l)
		
	end
end

# ╔═╡ a955bf69-2070-448c-960b-a5c893cc3c5e
begin
	# training (pay attention to the syntax)
	
	losses3 = [] # history of losses
	
	for _ in 1:n_epochs
	    
	    # complete training
	    Flux.train!(loss3, model2, data_train_mlp, optimizer2)
	    # Accumulate losses
	    l = loss3(model3, x_train, y_train)
	    push!(losses3, l)
		
	end
end

# ╔═╡ 1967ac1c-99f6-4a8c-8cb4-da85e6711a4a
begin
	preditions_trainingMLP = first.(model.(x_train))
	predictions_testMLP = first.(model.(x_test))
end

# ╔═╡ 2f9bc16b-4555-40c4-b991-43045deb7dd4
begin
	preditions_trainingMLP2 = first.(model2.(x_train))
	predictions_testMLP2 = first.(model2.(x_test))
end

# ╔═╡ c4307bb9-cb93-4ff2-a38c-20526401afcd
begin
	preditions_trainingMLP3 = first.(model3.(x_train))
	predictions_testMLP3 = first.(model3.(x_test))
end

# ╔═╡ b595e183-e6ea-42bb-afdc-ae3d8314143f
begin
	threshold = 0.5
...
# ╔═╡ d749ac27-5fad-4968-a3b4-31d07b573b09
begin
	threshold2 = 0.5
	println("Accuracy (training): ", compute_accuracy(preditions_trainingMLP2, y_train, threshold))
	println("Accuracy (test): ", compute_accuracy(predictions_testMLP2, y_test, threshold))	
end

# ╔═╡ af841213-b6be-4185-a4a2-0e82041a6d59
begin
	threshold3 = 0.5
	println("Accuracy (training): ", compute_accuracy(preditions_trainingMLP3, y_train, threshold))
	println("Accuracy (test): ", compute_accuracy(predictions_testMLP3, y_test, threshold))	
end

# ╔═╡ 3c2b7b3b-e4f3-4363-843e-e55b84ba5fd5
pred1=model.predict_proba(x_test)

# ╔═╡ e7fd2f49-6144-4b39-9e89-b212b09ba9e0
pred2=model2.predict_proba(x_test)

# ╔═╡ 92d622e7-9a30-4e34-abe4-9e35eae06562
pred3=model3.predict_proba(x_test)

# ╔═╡ e61cbdd7-70eb-42ba-bad7-a7c116a18660
print(model)
...
CombineML = "f3117721-c1d3-5ff8-9230-e4dd1549a252"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "e32b54557072e903309acb17fc9ad31734422d85"
...
[[deps.CombineML]]
deps = ["DelimitedFiles", "Distributed", "InteractiveUtils", "LinearAlgebra", "Random", "Statistics", "Test"]
git-tree-sha1 = "df2020686dbc7959e0a75d90cd028feeb14eac37"
uuid = "f3117721-c1d3-5ff8-9230-e4dd1549a252"
version = "1.2.0"
